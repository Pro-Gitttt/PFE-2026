...
public class ExecutionTrackingService {
    ...
    private final ExecutionUpdateService executionUpdateService; // ✅ injected external service
    ...
    public void trackAsync(Long executionId, Long pipelineId, String queueId) {
        ...
        // ================= TIMEOUT WAITING FOR BUILD NUMBER =================
        if (buildNumber == null) {
            log.error("[TRACK] Could not resolve build number for execution {}", executionId);
            executionUpdateService.updateStatus(executionId, PipelineStatus.FAILED); // ✅ goes through proxy
            return;
        }

        // ================= MARK AS RUNNING =================
        executionUpdateService.updateRunning(executionId, pipeline, buildNumber); // ✅ goes through proxy
        ...
        // ================= SAVE FINAL STATUS =================
        executionUpdateService.updateFinal(executionId, status); // ✅ goes through proxy

        log.info("[TRACK] Execution {} finished with {}", executionId, status);
    }

    // ================= SAFE LOAD (waits for flush from caller) =================
