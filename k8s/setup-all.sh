...
for NS in infra dev staging prod; do
  kubectl create secret generic db-secret \
    ...
    --from-literal=DB_PASSWORD=DevSecOps2024! \
...
for NS in infra apps dev staging prod; do
  kubectl create secret generic jwt-secret \
    --from-literal=JWT_SECRET=stb-devsecops-jwt-secret-2024-very-long-key \
    -n "$NS" --dry-run=client -o yaml | kubectl apply -f -
done

# mail-secret (copy to all app namespaces)
kubectl apply -f "${SCRIPT_DIR}/secrets/mail-secret.yaml"
for NS in dev staging prod; do
  kubectl get secret mail-secret -n apps -o yaml \
    | sed "s/namespace: apps/namespace: $NS/" \
