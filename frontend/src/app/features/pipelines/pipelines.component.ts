    ...
    this.promSvc.snapshot().subscribe({
      next: snap => {
        ...
        this.rpsHistory.push({
          time: now,
          value: snap.httpRps.reduce((a, b) => a + b.value, 0),
        });
        this.cpuHistory.push({
          time: now,
          value: (snap.cpuUsage.reduce((a, b) => a + b.value, 0) / Math.max(1, snap.cpuUsage.length)) * 100,
        });
        this.errorHistory.push({
          time: now,
          value: snap.httpErrors.reduce((a, b) => a + b.value, 0),
        });
